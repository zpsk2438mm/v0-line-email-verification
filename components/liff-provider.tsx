...
interface LiffContextType {
  ...
  userProfile: UserProfile | null; // 新增：保存 LINE 個人資料
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-4">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-xl font-bold">南台二手物平台</h1>
        <p className="text-slate-500 text-sm">請驗證您的學校信箱以繼續</p>
      </div>

      <div className="w-full max-w-sm bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div className="space-y-4">
          ...
          <Button onClick={handleSendCode} className="w-full h-11 rounded-xl font-bold bg-blue-600">
            <ShieldCheck className="w-4 h-4 mr-2" /> 發送驗證碼
          </Button>
          <p className="text-[10px] text-center text-slate-400 pt-2 font-medium tracking-widest">僅限南台科技大學師生使用</p>
        </div>
      </div>

      {/* 提示 Dialog (驗證碼 123456) */}
        ...
        <DialogContent className="rounded-3xl max-w-[90%] sm:max-w-sm">
          <DialogHeader className="items-center">
            ...
            <DialogDescription className="text-center">
              測試環境驗證碼：<br/>
              <span className="text-3xl font-black text-blue-600 tracking-tighter">{VERIFICATION_CODE}</span>
            </DialogDescription>
          </DialogHeader>
          <Button onClick={() => { setShowTipDialog(false); setShowOtpDialog(true); }} className="w-full rounded-xl h-12">前往輸入</Button>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showOtpDialog} onOpenChange={setShowOtpDialog}>
        <DialogContent className="rounded-3xl max-w-[90%] sm:max-w-sm">
          <DialogHeader className="items-center text-center">
            ...
            <DialogDescription>請輸入發送至 {schoolEmail} 的驗證碼</DialogDescription>
          </DialogHeader>
          <div className="flex flex-col items-center gap-6 py-4">
            <InputOTP maxLength={6} value={otpValue} onChange={setOtpValue}>
              <InputOTPGroup className="gap-2">
                {[...Array(6)].map((_, i) => <InputOTPSlot key={i} index={i} className="w-10 h-14 text-xl font-bold rounded-xl" />)}
              </InputOTPGroup>
            </InputOTP>
            <Button 
              onClick={() => otpValue === VERIFICATION_CODE ? onVerified(schoolEmail) : setError("驗證碼錯誤")} 
